...
When the last adapter for a root is collected, it closes the shared Chroma client before releasing process ownership. Active calls and snapshot contexts keep their adapter alive through completion. The root can then be reopened in the same or another process; persisted indexes remain intact. If client shutdown fails, ownership remains held and reopening is rejected until process exit.
